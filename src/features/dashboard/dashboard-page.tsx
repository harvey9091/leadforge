...
/**
 * Dashboard page — Phase 2: real discovery data.
 *
 * Pulls from /api/v1/discover/stats for real metrics:
 *  - Companies discovered (total + today)
 *  - Jobs today, running, completed
 *  - Average runtime
 *  - Source distribution
 *  - Recent discoveries
 *
 * No placeholder data — everything comes from the database.
 */
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        ...
        <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: 0.08 }}>
          <StatCard
            ...
            format="raw"
            icon={stats?.jobs.running ? Loader2 : Activity}
            loading={isLoading}
          />
        </motion.div>
        ...
        <Card className="lg:col-span-2 p-5 border-border/60 bg-card/40">
          ...
          {isLoading ? (
            ...
            <div className="h-[240px] flex items-center justify-center text-[13px] text-muted-foreground">
              No data yet — run a discovery job to see metrics
            </div>
          ) : (
        ...
        <Card className="p-5 border-border/60 bg-card/40">
          ...
          {enrichLoading ? (
            ...
            <div className="space-y-3">
              ...
              <StatRow label="Avg crawl time" value={enrichStats && enrichStats.avgCrawlMs > 0 ? `${(enrichStats.avgCrawlMs / 1000).toFixed(1)}s` : "—"} icon={Activity} />
              <div className="pt-3 mt-3 border-t border-border/60">
                <div className="flex items-center justify-between text-[11.5px]">
      ...
      {aiStats && aiStats.total > 0 && (
        <Card className="p-5 border-border/60 bg-card/40 mb-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <AIMetric label="Avg Video Opp" value={`${aiStats.avgVideoOpportunity}`} tone="info" />
        ...
        {isLoading ? (
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-[13px] text-muted-foreground">
              ...
              <a href={routeHref("discover")} className="text-foreground font-medium hover:underline">
...
function StatRow({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-[15px] font-semibold tabular-nums text-foreground">{formatNumber(value)}</span>
    </div>
  );
}
